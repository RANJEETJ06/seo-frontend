...
import { apiErrorMessage, searchConsoleApi } from "../api";

/**
 * /oauth/google/search-console/callback
 *
 * Google redirects here with `?code=...&state=...` after the Search Console
 * consent screen. We post both to the backend, which exchanges the code and
 * persists the org's GSCConnection. The strict-mode ref guard prevents the
 * React 18 dev double-submit that would make Google reject the second call
 * with "invalid_grant".
 */
const OAuthSearchConsoleCallback = () => {
  const navigate = useNavigate();
  const [status, setStatus] = useState<"connecting" | "ok" | "error">("connecting");
  const [message, setMessage] = useState<string>("Connecting your Search Console account…");
  ...
  useEffect(() => {
    ...
    searchConsoleApi
      ...
      .then((conn) => {
        ...
        setTimeout(() => navigate("/search-console", { replace: true }), 1500);
      })
      .catch((err) => {
        setStatus("error");
        setMessage(apiErrorMessage(err, "Could not connect Search Console account"));
      });
  }, [navigate]);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-panel-2 p-6">
      <div className="w-full max-w-md rounded-lg border border-line-2 bg-panel p-6 shadow-sm">
        <h1 className="text-lg font-semibold text-ink">Search Console connection</h1>
...
export default OAuthSearchConsoleCallback;
